defmodule Twittex.Workers.ProfileWorker do
  ...
  def start_link(profile_state) do
    ...
    save_thought(profile_state.profile, :waking_up, [])
    {:ok, pid}
  end
  ...
  defp generate_post(profile) do
    ...
    save_thought(profile, :post, tweet: tweet)
    schedule_action()
  end
  ...
  defp read_timeline(profile) do
    ...
    save_thought(profile, :read_timeline, [])
  end
  ...
  defp read_comments(profile) do
    ...
    save_thought(profile, :read_comments, [])
  end

  defp save_thought(profile, :waking_up, _opts) do
    Accounts.create_thought(%{profile_id: profile.id, text: "Profile is waking up..."})
  end

  defp save_thought(profile, :post, opts) do
    Accounts.create_thought(%{profile_id: profile.id, text: "Profile just tweeted: #{opts[:tweet].text}"})
  end

  defp save_thought(profile, :read_comments, _opts) do
    Accounts.create_thought(%{profile_id: profile.id, text: "Profile is reading comments in their posts..."})
  end

  defp save_thought(profile, :read_timeline, _opts) do
    Accounts.create_thought(%{profile_id: profile.id, text: "Profile is reading the timeline..."})
  end
